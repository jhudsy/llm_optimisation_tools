#!/usr/bin/env python3
"""Run the canonical farming prompt via Ollama over stdio or HTTP MCP transports for MiniZinc."""
...
HTTP_PORT = int(os.environ.get("OPTIMISE_MCP_HTTP_PORT", "8766"))
...
SYSTEM_PROMPT = dedent(
        """
         ...
         2. Produce a complete MiniZinc model (.mzn) string using EXACT MiniZinc syntax:
         
            CRITICAL SYNTAX RULES:
            - ONE variable per line: "var 0..110: wheat;" then "var 0..110: corn;" (NOT "var 0..110: wheat, corn;")
            - NO double types: "var 0..110: wheat;" (NOT "var 0..110: int: wheat;")
            - Constraints: "constraint 3*wheat + 2*corn <= 240;"
            - Objective: "var float: profit;" then "constraint profit = 40*wheat + 30*corn;"
            - Solve: "solve maximize profit;"
            - Output: "output [\"wheat=\", show(wheat), \" corn=\", show(corn), \" profit=\", show(profit)];"
          ...
          3. Call validate_minizinc to check the syntax. If validation fails, fix errors and revalidate.
          4. Call solve_minizinc with that MiniZinc text (and optional time_limit). Report solver status,
                objective value, and the key decision variable assignments verbatim from the tool.
          5. After successful solve, respond with the final answer.
          Never invent other tools; the server only offers validate_minizinc and solve_minizinc.
        """
).strip()
...
USER_PROMPT = dedent(
    ...
    Use the optimise-mcp tools to translate this brief into MiniZinc format, validate it, solve it, 
    and deliver the optimal acreage plan. Embed the entire .mzn body inside the tool call arguments. 
    Share each tool result verbatim before giving the final plan—never stop after the first tool.
    """
).strip()

SOLVE_REMINDER = dedent(
    """
    You must call solve_minizinc with a valid MiniZinc body before delivering
...
PLAN_SCHEMA = dedent(
        ...
        Respond **only** with valid JSON following this schema:
        {
            ...
            "tool_name": "validate_minizinc" | "solve_minizinc" (required when action is call_tool),
...
def validate_minizinc_body(mzn_text: str) -> tuple[bool, str | None]:
    """Check for the key sections required by the MiniZinc solver before invoking solve_minizinc."""

    stripped = (mzn_text or "").strip()
    if not stripped:
        return False, "MiniZinc payload is empty. Provide the full .mzn text."
    lower_blob = stripped.lower()
    if "var " not in lower_blob:
        return False, "MiniZinc must declare at least one decision variable (e.g., 'var 0..100: x;')."
    if "solve" not in lower_blob:
        return False, "Include a 'solve' statement (e.g., 'solve maximize ...' or 'solve minimize ...')."
    if not ("maximize" in lower_blob or "minimize" in lower_blob):
        return False, "The solve statement must specify 'maximize' or 'minimize'."
    return True, None
...
async def run_roundtrip(
    ...
    async with connection_context(transport, HTTP_HOST, HTTP_PORT) as connection:
        client = MultiServerMCPClient({"minizinc-mcp": connection})

        if verbose:
            if transport == "http":
                print(f"Connecting to minizinc-mcp via HTTP on {HTTP_HOST}:{HTTP_PORT}...")
            else:
                print("Connecting to minizinc-mcp via stdio...")

        async with client.session("minizinc-mcp") as session:
            if verbose:
                print("Initialising MCP session and loading tools...")
            tools = await load_mcp_tools(
                session,
                callbacks=client.callbacks,
                server_name="minizinc-mcp",
            )
            if verbose:
                print(f"Loaded {len(tools)} tools from minizinc-mcp.")
            ...
            for turn in range(12):
                ...
                if plan is None or "action" not in plan:
                    ...
                    if invalid_attempts >= 3:
                        return {
                            ...
                            "solve_minizinc_seen": solve_minizinc_seen,
                        }
                    messages.append(
                        HumanMessage(
                            content=(
                ...
                if action == "call_tool":
                    ...
                    if tool_name == "solve_minizinc":
                        if solve_minizinc_seen:
                            messages.append(
                                HumanMessage(
                                    content=(
                                        "You already solved the constraint program successfully. Send a final respond action summarising"
                                        " the objective value and decision variables instead of calling solve_minizinc again."
                                    )
                                )
                            )
                            continue
                        ...
                        if not isinstance(mzn_body, str) or not mzn_body.strip():
                            messages.append(
                                HumanMessage(
                                    content=(
                                        "solve_minizinc requires a non-empty 'mzn_code' string containing the full MiniZinc model."
                                        " Embed the entire .mzn content directly in the arguments."
                                    )
                                )
                            )
                            continue
                        mzn_ok, mzn_error = validate_minizinc_body(mzn_body)
                        if not mzn_ok:
                            messages.append(
                                HumanMessage(
                                    content=(
                                        "The provided mzn_code is not valid MiniZinc syntax: "
                                        f"{mzn_error}. Include variable declarations, constraints, and a solve statement."
                                    )
                                )
                            )
                            continue
                    ...
                    if tool_name == "solve_minizinc" and isinstance(artifact, dict):
                    ...
                    if (
                        ...
                        messages.append(
                            HumanMessage(
                                content=(
                                    "solve_minizinc succeeded. Now emit a JSON plan with action 'respond' that summarises the"
...
def render_result(result: dict[str, Any]) -> None:
    ...
    if not result.get("solve_minizinc_seen", False):
        print("\nWarning: no successful solve_minizinc call was observed.")
...
def main(
    ...
    *,
    ...
    parser.add_argument(
        ...
        help="MCP transport to use (stdio launches minizinc-mcp over pipes, http starts the FastMCP server).",
    )
